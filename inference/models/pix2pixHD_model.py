...
class Pix2PixHDModel(BaseModel):
    ...
    def encode_input(self, label_map, clothes_mask, all_clothes_label):
        ...
        input_label = input_label.scatter_(1, label_map, 1)

        masked_label = torch.cuda.FloatTensor(torch.Size(oneHot_size)).zero_()
        masked_label = masked_label.scatter_(1, (label_map * (1 - clothes_mask)).data.long().cuda(), 1)

        c_label = torch.cuda.FloatTensor(torch.Size(oneHot_size)).zero_()
        all_clothes_label = all_clothes_label.long()
        c_label = c_label.scatter_(1, all_clothes_label, 1)
