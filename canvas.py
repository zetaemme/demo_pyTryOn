...
class main:
    ...
    def save(self):
        ...
        if file:
            ...
            back_to_grayscale(PIL.ImageGrab.grab().crop((x + 2, y + 2, x1 - 2, y1 - 2))).save(file + '.png')

            files = glob.glob('./Data_preprocessing/temp/*')
            for f in files:
                os.remove(f)
    ...
    def importImage(self):
        ...
        rgb_image.save('./Data_preprocessing/temp/' + self.file[len(self.file)-12:])
        self.image2 = ImageTk.PhotoImage(file='./Data_preprocessing/temp/' + self.file[len(self.file) - 12:])
...
if __name__ == '__main__':
	...
	if len([name for name in os.listdir('./Data_preprocessing/test_label') if os.path.isfile(name)]) > 5:
